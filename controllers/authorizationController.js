...
const Register = async (request, response, next) => {
  ...
  if (!password.match(/(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[^A-Za-z0-9])(?=.{8,})/))
    throw {
      ...
      message: 'Password must contain at least 1 uppercase letter, 1 lowercase letter, 1 special charater and 1 digit!'
    }

  const userAlreadyExists = await User.findOne({ phoneNumber1 }).catch(error => next(error))
...
const Login = async (request, response, next) => {
  ...
  const user = await User.findOne({ phoneNumber1: phoneNumber }).select('+password').catch(error => next(error))
  if (!user)
